...
class OutlierDetector:
    ...
    def prepare_summary_table(self, outlier_indices):
        """
        Prepare summary table for outlier detection including number of outliers and outliers ratios
        for Z-score calculation for each numeric column separately.
        ...
        self.outlier_summary_df = outliers_df._append({"Column name": "Average", "Number of outliers": outliers_mean,
                                                       "Outliers ratio": outliers_percent_mean},
                                                      ignore_index=True).fillna(0)

    def update_summary_table(self, outlier_dict):
        """
        Update summary table with number of outliers and outliers ratio when an anomaly detection is performed
        using one of available algorithms
        :param outlier_dict: dictionary representing row to be added to summary table
        :return:
        """
    ...
    def run_isolation_forest(self, **kwargs):
        ...
        print(
            f"Isolation Forest outlier ratio: {(len(np.where(y == -1)[0]) / self.df.shape[0]) * 100:.2f}% ({len(np.where(y == -1)[0])} outliers found)")
    ...
    def run_knn(self, outlier_ratio=.01, **kwargs):
        ...
        kneedle = KneeLocator(np.arange(len(distances_sorted)), distances_sorted, curve="convex",
    ...
    def run_local_outlier_factor(self, **kwargs):
        ...
        print(
            f"Local outlier factor outlier ratio: {(len(np.where(scores == -1)[0]) / self.df.shape[0]) * 100:.2f}% ({len(np.where(scores == -1)[0])} outliers found)")
    ...
    def run_dbscan(self, implementation="sklearn", **kwargs):
        """
        ...
        if kwargs["eps"] == 'knn':
            knn = NearestNeighbors(n_neighbors=2 * self.df_preprocessed.shape[1] - 1)
            knn.fit(self.df_preprocessed.dropna())
            distances, indexes = knn.kneighbors(self.df_preprocessed.dropna())
            distances_mean = np.mean(distances, axis=1)
            distances_sorted = np.sort(distances_mean)
            ...
            # self.visualizer.plot_k_distance(distances_sorted, kneedle.knee)
        if kwargs["min_samples"] == 'n_features':
            kwargs["min_samples"] = self.df_preprocessed.shape[1]
        print(kwargs)
        if implementation == "dbscan":
            start_time = time()
            scores, core_samples_mask = dbscan.DBSCAN(self.df_preprocessed, kwargs["eps"],
            ...
            model.fit(self.df_preprocessed)
            scores = model.labels_
            end_time = time()
        outliers = np.where(scores == -1)[0]
        print(
            f"DBSACAN outlier ratio: {(len(np.where(scores == -1)[0]) / self.df.shape[0]) * 100:.2f}% ({len(np.where(scores == -1)[0])} outliers found)")
        ...
        return outliers, round(end_time - start_time, 2)
