...
class PandasValidator:
    def __init__(self, df, data_path):
        ...
        self.linters_dict = {"duplicates":0, "missing_values":1, "num_negative_values":2, "num_small_mode":3,
                             ...
                             "obj_long_max":12, "lang_special_chars":13, "lang_numeric_vals":14, "lang_oov_invalid":15,
                             "lang_oov_custom":16, "lang_abbr":17, "time_format":18, "time_monotonic":19,
                             "time_not_uniform":20, "time_long":21}
        self.linter_count = np.zeros(21).astype(int)
    ...
    def categorical_values_check(self, categories_df):
        ...
        :return:
        """
        self.language_validator = LanguageValidator(categories_df, self.data_path, self.visualizer, self.linter_count, self.linters_dict)
    ...
    def print_categorical_summary(self, summary_df, categorical_columns):
        ...
        if any(unique_categories):
            ...
            self.linter_count[self.linters_dict["obj_unique"]] += len(idx)
        if any(single_category):
            ...
            warnings.warn(
                ...
                category=ObjectDataWarning, stacklevel=4)
            self.linter_count[self.linters_dict["obj_same"]] += len(idx)
    ...
    def prepare_categorical_summary_df(self, categorical_columns):
        summary_df = self.df[categorical_columns].apply(
            lambda x: [len(x.value_counts()), list(x.value_counts().index),
                       list(x.value_counts().values), np.mean(list(map(lambda y: len(y), list(x.value_counts().index)))),
    ...
    def prepare_numeric_summary(self, numeric_columns):
        ...
        unique_difference = np.unique(row_differences, axis=0)
        min_row_difference = np.nanmin(unique_difference, axis=0)
        diff_df = pd.DataFrame([min_row_difference, max_row_difference],
                               columns=numeric_columns, index=['min_row_diff', 'max_row_diff'])
        summary_df = pd.concat([summary_df, additional_quantiles, mode_df, diff_df, skewed_df], axis=0, ignore_index=False,
                               keys=None)
    ...
    def print_numeric_summary(self, summary_df):
        ...
        wide_range = np.where(summary_df.loc['max'] - summary_df.loc['min'] > 6*(summary_df.loc['75%'] - summary_df.loc['25%']))[0]
        ...
        if any(high_row_diff):
            warnings.warn(f"Columns with high row difference between two following sorted elements: {', '.join(summary_df.columns[high_row_diff])}",
                          category=NumericValuesWarning, stacklevel=4)
            affected_columns.append(high_row_diff)
            self.linter_count[self.linters_dict["num_max_gap"]] += len(high_row_diff)
        if any(zero_mode):
            warnings.warn(f"Columns with modal value occurrence less than 1%: {', '.join(summary_df.columns[zero_mode])}",category=NumericValuesWarning, stacklevel=4)
    ...
    def run_datetime_validator(self):
        ...
        if any(date_columns):
            ...
            for column in date_columns:
                ...
                if delta.years >= 20:
                    warnings.warn(f"Long period of datetime data: {delta.years} years", category=DateWarning,stacklevel=4)
                ...
                if self.df[column].is_monotonic_increasing or self.df[column].is_monotonic_decreasing:
                    warnings.warn(f"Datetime column is monotonic - it is probably important", category=DateWarning, stacklevel=4)
